...
func LoadVCE(slotnum byte, vce []byte) (err error) {
	...
	if err = conn.WriteByteWithTimeout(TIMEOUT_MS, slotnum, "write slotnum"); err != nil {
		return
	}
	var status byte
	if status, err = conn.ReadByteWithTimeout(TIMEOUT_MS, "read slotnum ACK"); err != nil {
		return
	}
	if status != DC1 {
		// slot error
	...
	if err = conn.WriteBytesWithTimeout(LONG_TIMEOUT_MS, vce, "write VCE"); err != nil {
		return
	}
	if status, err = conn.ReadByteWithTimeout(LONG_TIMEOUT_MS, "read VCE ACK"); err != nil {
		return
	}
	if status == ACK {
		// done - no filters
...
func LoadCRTBytes(crt []byte) (err error) {
	...
	// LOB of the length
	calcCRCByte(lenLob)
	if err = conn.WriteByteWithTimeout(TIMEOUT_MS, lenLob, "write length LOB"); err != nil {
		return
	}
	// HOB of the length
	calcCRCByte(lenHob)
	if err = conn.WriteByteWithTimeout(TIMEOUT_MS, lenHob, "write length HOB"); err != nil {
		return
	}

	calcCRCBytes(crt)

	if err = conn.WriteBytesWithTimeout(LONG_TIMEOUT_MS, crt, "write CRT bytes"); err != nil {
		return
	}
	...
	// HOB of the crc
	if err = conn.WriteByteWithTimeout(TIMEOUT_MS, crcHob, "write CRC HOB"); err != nil {
		return
	}
	// LOB of the crc
	if err = conn.WriteByteWithTimeout(TIMEOUT_MS, crcLob, "write CRC LOB"); err != nil {
		return
	}

	var status byte
	if status, err = conn.ReadByteWithTimeout(LONG_TIMEOUT_MS, "read CRT ACK"); err != nil {
		return
	}
	if status == ACK {
		// errors will implicitly show  up in the log but we need to explicitly log success
...
func LoadSYN(bytes []byte) (err error) {
	...
	if err = conn.WriteBytesWithTimeout(LONG_TIMEOUT_MS, bytes, "SYN bytes"); err != nil {
		return
	}
	// expect an ACK:
	var status byte
	if status, err = conn.ReadByteWithTimeout(LONG_TIMEOUT_MS, "read SYN ACK"); err != nil {
		return
	}
	if status == ACK {
		// errors will implicitly show  up in the log but we need to explicitly log success
...
func SaveSYN() (bytes []byte, err error) {
	...
	var len_buf []byte
	if len_buf, err = conn.ReadBytesWithTimeout(TIMEOUT_MS, 2, "read CMOS length"); err != nil {
		return
	}
	...
	var cmos_buf []byte
	if cmos_buf, err = conn.ReadBytesWithTimeout(LONG_TIMEOUT_MS, cmos_len*2+2, "read CMOS"); err != nil {
		return
	}
	...
	if seq_len != 0 {
		if seq_buf, err = conn.ReadBytesWithTimeout(LONG_TIMEOUT_MS, seq_len, "read SEQ"); err != nil {
			return
		}
	}
	var crc_buf []byte
