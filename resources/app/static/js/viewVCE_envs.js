...
let viewVCE_envs = {
    ...
    init: function (incrementalUpdate) {
        ...
        viewVCE_envs.floatAmpLowVal = [];
        viewVCE_envs.floatAmpUpVal = [];
    ...
    initFloatVals: function () {
        if (viewVCE_envs.floatAmpLowVal.length == 0) {
            for (i = 1; i <= 16; i++) {
                var input = document.getElementById(`envAmpLowVal[${i}]`)
                if (input && (window.getComputedStyle(input).display != "none")) {
                    var oldval = parseInt(input.value, 10)
                    viewVCE_envs.floatAmpLowVal.push(oldval)
                }
                input = document.getElementById(`envAmpUpVal[${i}]`)
                if (input && (window.getComputedStyle(input).display != "none")) {
                    var oldval = parseInt(input.value, 10)
                    viewVCE_envs.floatAmpUpVal.push(oldval)
                }
            }
        }
    },
    ...
    onchangeGain: function (ele) {
        if (ele.id.match(/Low/)) {
            stub = 'envAmpLowVal'
            floatVals = viewVCE_envs.floatAmpLowVal;
        } else {
            stub = 'envAmpUpVal'
            floatVals = viewVCE_envs.floatAmpUpVal;
        }
        ...
        for (i = 1; i <= 16; i++) {
            ...
            if (input && (window.getComputedStyle(input).display != "none")) {
                //var oldval = parseInt(input.value,10)
                // one-based vs. zero-base:
                var oldval = floatVals[i-1]
                var delta = oldval * gain;
                var floatNewVal = oldval + delta;
                var newval = Math.round(floatNewVal);
                floatVals[i-1] = floatNewVal;
                /*
                if (stub === 'envAmpLowVal') {
                    viewVCE_envs.floatAmpLowVal[i-1] = floatNewVal;
                } else {
                    viewVCE_envs.floatAmpUpVal[i-1] = floatNewVal;
                }
                 */
                console.log(" gain " + gain + " " + stub + " " + input.id + " " + oldval + " " + delta + " " + floatVals[i] + " " + newval);
                input.value = "" + newval;
                viewVCE_envs.onchange(input, floatNewVal);
            }
        }
    }
    ,
...
copyFrom: function (fromOsc) {
    ...
    if (viewVCE_envs.supressOnchange) { /*console.log("viewVCE_envs.suppressOnChange");*/
        return;
    }
...
deb_onchangeEnvAccel: null,
    ...
    raw_onchangeEnvAccel
:

function (ele) {
    if (viewVCE.supressOnchange) { /*console.log("raw viewVCE.suppressOnChange");*/
        return;
    }
    if (viewVCE_envs.supressOnchange) { /*console.log("raw viewVCE_envs.suppressOnChange");*/
        return;
    }
...
onchange: function (ele, extraarg) {
    if (viewVCE.supressOnchange) { /*console.log("viewVCE.suppressOnChange");*/
        return;
    }
    if (viewVCE_envs.supressOnchange) { /*console.log("viewVCE_envs.suppressOnChange");*/
        ...
        switch (fieldType) {
            ...
            case "envAmpLowVal":
                ...
                if (extraarg == undefined) {
                    ...
                    viewVCE_envs.initFloatVals();
                    viewVCE_envs.floatAmpLowVal[eleIndex - 1] = value;
                }
                break;
            case "envAmpUpVal":
                bytevalue = viewVCE_envs.unscaleAmpEnvValue(value);
                ...
                if (extraarg == undefined) {
                    // update the floatVal so gains work
                    viewVCE_envs.initFloatVals();
                    viewVCE_envs.floatAmpUpVal[eleIndex - 1] = value;
                }
                break;
            case "envAmpLowTime":
...
function (oscNum, envNum, animate) {
