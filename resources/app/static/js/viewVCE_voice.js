...
`;
	...
	_withZeroconf: function (prompt1, prompt2, zeroconfSelector, actionAfterSelect, recurseAction, successCallback) {
		...
		console.log("top withZeroconf " + zeroconfSelector + " " + actionAfterSelect);

		let message = {
			"name": zeroconfSelector,
			"payload": ""
		};
		astilectron.sendMessage(message, function (message) {
			if (message.name === "error") {
	...
	raw_connectSynergy: function (zeroconfChoice, ignored, callback) {
		let message = {
			"name": "connectSynergy",
			"payload": {
				"ZeroconfChoice": zeroconfChoice // optional param - null unless user just selected from a menu
			}
		};
