...
// a handler we can redirect to a file, but which uses the simple default log format (not key=value as the slog.NewTextHandler does)
// adapted from https://github.com/jba/slog/blob/main/handlers/loghandler/log_handler.go
...
func NewSimpleHandler(w io.Writer, opts *slog.HandlerOptions) *Handler {
	h := &Handler{w: w}
	if opts != nil {
		h.opts = *opts
	}
