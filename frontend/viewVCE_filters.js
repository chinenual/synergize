...
export let viewVCE_filters = {
	...
	raw_onchange: function (ele, updateChart) {
		...
		let filterIndex = parseInt(selectEle.value, 10); // index into the uncompressedFilters array
	...
	uncompressFilters: function () {
		// the first time we evaluate this viewVCE.vce, the Filters may be compressed.  To make it easier to add/remove 
	...
	init: function (incrementalUpdate) {
		...
		if (viewVCE_filters.deb_copyFrom == null) {
			viewVCE_filters.deb_copyFrom = _.debounce(viewVCE_filters.raw_copyFrom, index.DEBOUNCE_WAIT);
		}
		...
		// Option values are index into the viewVCE.vce.Head.FILTERS array (and one extra with value -1 for "All")
	...
	raw_copyFrom: function (fromFilterIndex, fromFilterName) {
		...
		let toFilterIndex = filterSelectEle.options[filterSelectEle.selectedIndex].value;
		...
		let message = {
			...
			"payload": {
				...
				"Values": viewVCE.vce.Extra.uncompressedFilters[fromFilterIndex]
			}
		};
		index.spinnerOn();
		astilectron.sendMessage(message, function (message) {
			...
			if (message.name === "error") {
				...
				for (let i = 0; i < 32; i++) {
					viewVCE.vce.Extra.uncompressedFilters[toFilterIndex][i] = viewVCE.vce.Extra.uncompressedFilters[fromFilterIndex][i]
				}
	...
	filtersChartUpdate: function (filterIndex, filterName, animate) {
		...
		if (filterIndex >= 0) {
			...
			$('#filterTable td.val input').each(function (i, obj) {
				...
				if (animate) {
				   viewVCE_voice.sendToCSurface(obj, id, viewVCE.vce.Extra.uncompressedFilters[filterIndex][idx]);
				}
			});
			...
			if (filterIndex > 0) {
				...
				for (let i = 0; i <= viewVCE.vce.Head.VOITAB; i++) {
					if (filterIndex == viewVCE.vce.Head.FILTER[i]) {
						color = viewVCE.chartColors[(colorIdx) % viewVCE.chartColors.length]
						break;
			...
			$('#filterTable').hide();
			// only include the ones actually in use (see viewVCE.vce.Head.FILTER entry)
			for (let i = 0; i <= viewVCE.vce.Head.VOITAB; i++) {
					...
					datasets.push(
						{
							...
							backgroundColor: color,
							borderColor: color,
							data: viewVCE.vce.Extra.uncompressedFilters[0] // ZEROTH ele is the a-filter
						}
					);
				}
			}
			...
			for (let i = 0; i <= viewVCE.vce.Head.VOITAB; i++) {
				...
				if (viewVCE.vce.Head.FILTER[i] > 0) {
					...
					let color = viewVCE.chartColors[colorIdx % viewVCE.chartColors.length];
					colorIdx++;
					datasets.push(
						{
							fill: false,
							...
							backgroundColor: color,
							borderColor: color,
							data: viewVCE.vce.Extra.uncompressedFilters[i + 1] // b-filters by one-based osc value
						}
					);
				}
			}
