...
export let viewVCE_keyprop = {
  ...
  raw_onchange: async function(ele, updateChart) {
    ...
    try {
      await UIService.SetVoiceKPROPEle(eleIndex, value);
    } catch (exc) {
  ...
  init: function(incrementalUpdate) {
    ...
    if (viewVCE_keyprop.deb_onchange == null) {
      // viewVCE_keyprop.deb_onchange =
      // index.debounceFirstArg(viewVCE_keyprop.raw_onchange,
      // DEBOUNCE_WAIT_SHORT);
      ...
      options: {
        ...
        scales: {
          xAxes: [{
            ...
            ticks: {min: 0, max: 23, color: '#666', display: true}
          }],
          yAxes: [{
            id: 'y-axis',
            gridLines: {
            ...
            scaleLabel: {display: true, labelString: 'Bounds'},
            ticks: {min: 0, max: 32, stepSize: 4, color: '#eee', display: true}
          }],
        },
        responsive: false,
    ...
    if (viewVCE_voice.voicingMode) {
      viewVCE_chartdrag.init(
          viewVCE_keyprop.chart, viewVCE_keyprop.onchange, 'keyprop', 0, 23, 0,
          32);
    }
